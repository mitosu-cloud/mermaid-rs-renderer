...
pub(super) fn compute_c4_layout(graph: &Graph, config: &LayoutConfig) -> Layout {
    ...
    let viewbox_x = 0.0;
    let title_extra_y = if graph.diagram_title.is_some() {
        60.0
    } else {
        0.0
    };
    let viewbox_y = -(conf.diagram_margin_y + title_extra_y);
    let viewbox_width = width;
    let viewbox_height = height + title_extra_y;
